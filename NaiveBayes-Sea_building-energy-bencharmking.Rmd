...
The aim of this predictive model is to be able to classify buildings on the basis of being able to obtain Energy Star Certification (Energy Star Score >= 75). By training a Naive Bayes probabalistic model, we can classify other buildings in the Seattle area that have not enrolled in the energy star program, but have access to the same variables as our training and test sets.
...
mod_train = mod_train[2:6]
mod_test = mod_test[2:6]
```
...
In this cross table, we find an accuracy of ```{r}(33+253)/491``` or 58.25%. We have a ```{r}198/(198+33)``` or 85.7% false-negative rate and a ```{r}8/(8+253)``` or 3.1% false-positive rate as well. The extremely high false-negative rate is alarming, as this would result in an energy-star score for buildings that would exclude them from becoming energy star certified when in actuality they would meet the requirements.
