...
From this output, we see our adjusted R-squared increase to 0.1651 such that 16.51% of the variation in ENERGYSTARScore is explained by the model. While YearBuilt has become statistically significant at the 1% level, we see sign reversals for some key variables like NaturalGas.kBtu. This seems conterintuitive--an increase in natural gas usage increases the energy star score would be counter productive (we would expect this to be negative). From the low correlations in the exploritory sections and the low R-Squared values and switching signs for the coefficients from our output models, Regression appears to be a poor model for this information. 
